...
function usePersistentState<T>(key: string, initial: T) {
  ...
  useEffect(() => {
    try { const raw = localStorage.getItem(key); if (raw != null) setV(JSON.parse(raw)); } catch {}
  }, [key]);
  useEffect(() => {
...
function hasCubismCore(): boolean { return typeof (globalThis as any).Live2DCubismCore !== 'undefined'; }
async function ensureCubismCore(): Promise<void> {
  if (hasCubismCore()) return;
  await new Promise<void>((resolve, reject) => {
    const id = 'cubism-core-script';
    if (document.getElementById(id)) { setTimeout(() => (hasCubismCore() ? resolve() : reject(new Error('CubismCore not ready'))), 150); return; }
    const s = document.createElement('script');
    s.id = id; s.src = '/scripts/live2dcubismcore.min.js'; s.async = false;
    s.onload = () => resolve(); s.onerror = () => reject(new Error('Failed to load live2dcubismcore.min.js'));
...
function detectMouthParamId(model: any): string {
  ...
  try {
    ...
    const ids: string[] = [];
    for (let i=0;i<count;i++) { const id = core.getParameterId(i); if (id) ids.push(String(id)); }
...
export default function Studio() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try { Live2DModel = (await import('pixi-live2d-display-lipsyncpatch/cubism4')).Live2DModel; setLogs(p => [...p, '✅ using lipsyncpatch / cubism4']); }
      catch { const m:any = await import('pixi-live2d-display-lipsyncpatch'); Live2DModel = m.Live2DModel || m.Live2DModel4; setLogs(p => [...p, '⚠ fallback build']); }
      ...
      const ticker = PIXI.Ticker?.shared ?? app.ticker;
      const model = await Live2DModel.from(MODEL_URL, { ticker });
      ...
      const fit = () => {
        ...
        model.scale.set(s);
        model.x = (cw - model.width)/2; model.y = (ch - model.height)/2;
      };
      try { await new Promise<void>(r => { model.once?.('modelLoaded', ()=>r()); model.once?.('ready', ()=>r()); setTimeout(()=>r(),800); }); } catch {}
      ...
      (ticker?.add ? ticker.add : app.ticker.add)(tick);
  ...
  return (
    <div style={{display:'grid',gridTemplateColumns:'1fr 360px',height:'100vh',fontFamily:'ui-sans-serif,system-ui'}}>
      <div ref={stageRef} style={{position:'relative',background:'#f5f7fb'}}>
        <div style={{
          position:'absolute',left:12,top:12,background:'rgba(0,0,0,.55)',color:'#fff',
          padding:'8px 12px',borderRadius:8,fontSize:14,maxWidth:'60%'
        }}>
          {subtitle || '…'}
        </div>
      </div>
      <div style={{borderLeft:'1px solid #e5e7eb',display:'flex',flexDirection:'column'}}>
        <div style={{padding:12,borderBottom:'1px solid #e5e7eb',display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
          <div>
            <label>Speaker</label>
            <input type="number" min={1} value={speaker} onChange={e=>setSpeaker(parseInt(e.target.value||'1',10))} style={{width:'100%'}} />
          </div>
          <button onClick={() => playVoiceVox('テストです', speaker, (lv)=> (mouthTargetRef.current = lv))} style={{alignSelf:'end',padding:'8px 12px'}}>🔊 テスト再生</button>
        </div>
        ...
        <div style={{padding:12,display:'flex',gap:8}}>
          ...
          />
          <button onClick={handleSend} disabled={busy} style={{padding:'10px 16px',borderRadius:8,background: busy ? '#9ca3af':'#111827',color:'#fff'}}>送信</button>
        </div>
